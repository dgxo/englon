...
<head>
   ...
   <link rel="stylesheet" href="library/stylesheet.css?v1.1.2">
...
<body>
   ...
   <main>
      <div class="main">
         <h1>The Library - Paginated</h1>
         <h3>A huge collection of gаmes hosted on Englon,<br>so will hopefully never be blocked (again).</h3>
         <h3>Sorted into pages to avoid being entirely blocked and to help locate the possible offender.</h3>
         ...
         <ul class="desktop-ul links" id="links">
            <?php
            $directories = glob(getcwd() . '/library/*', GLOB_ONLYDIR);

            $offset = (isset($_GET['offset']) && is_numeric($_GET['offset']))
               ? intval(stripslashes($_GET['offset']))
               : 0;

            $quantity = 7;
            $page = ceil($offset / $quantity);
            $pages = ceil(count($directories) / $quantity);

            // get subset of file array
            $directories = array_slice($directories, $offset, $quantity);
            ...
            foreach ($directories as &$directory) {
               ...
               if ($dirname != 'library') {
                  echo '<li><a href="/gms/library/' . $dirname . '">' . $filtered_dirname . '</a></li>';
               }
            }
            ?>
         </ul>
         <br>
         <br>
         <a class="button<?= ($page <= 1) ? ' disabled' : '" href="?offset=' . $offset - $quantity ?>"><svg
               xmlns="http://www.w3.org/2000/svg" height="0.7em" fill="white"
               viewBox="0 0 448 512"><!--! Font Awesome Free 6.4.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2023 Fonticons, Inc. -->
               <path
                  d="M9.4 233.4c-12.5 12.5-12.5 32.8 0 45.3l160 160c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L109.2 288 416 288c17.7 0 32-14.3 32-32s-14.3-32-32-32l-306.7 0L214.6 118.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0l-160 160z">
            </svg></a>
         <?= $page ?>
         <a <?= ($page >= $pages - 1) ? 'style="cursor: not-allowed; background-color: #23232361"' : 'href="?offset=' . $offset + $quantity . '"' ?> class="button"><svg xmlns="http://www.w3.org/2000/svg" height="0.7em"
               fill="white"
               viewBox="0 0 420 512"><!--! Font Awesome Free 6.4.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2023 Fonticons, Inc. -->
               <path
